...
class physicsbox(object):
    def __init__(self, sess, width=128, height=64, image_size=128*64, 
                 batch_size=8, output_size=128*64,
                 gf_dim=128, df_dim=128, L1_lambda=10000, L2_lambda=1, 
                 input_c_dim=3, output_c_dim=1, condition_dim=6, overlap_dim=4,
    ...
    def build_model(self):
        ...
        #output channel
        self.output_structure = self.real_data[:, self.resolution*3:self.resolution*4] #output_structure ground truth
        ...
        if self.flag == 'stress' :
            self.real_B = self.vm_stress
        elif self.flag == 'strain' :
            self.real_B = self.strain_energy
        elif self.flag == 'out' :
            self.real_B = self.output_structure
        else:
        ...
        if self.flag == 'out':
            self.real_A = tf.reshape(tf.stack([self.vf,
                                               self.vm_stress,
                                               self.strain_energy,
                                               ], axis=-1), [-1, self.height, self.width, self.input_c_dim - 1])
        else:
            self.real_A = tf.reshape(tf.stack([self.vf,
                                         self.bc,
                                         self.loadx,
        ...
        if self.flag == 'stress':
            self.real_AB = tf.reshape(tf.stack([self.vf,
                                          ...
                                          self.loady,
                                          self.vm_stress,
                                          ], axis=-1), [-1, self.height, self.width, self.input_c_dim+self.output_c_dim])
            ...
            self.real_AB = tf.reshape(tf.stack([self.vf,
                                      ...
                                      [-1, self.height, self.width, self.input_c_dim + self.output_c_dim])
            self.fake_AB = tf.reshape(tf.stack([self.vf,
                                                self.bc,
                                                self.loadx,
                                                self.loady,
            ...
            self.real_AB = tf.reshape(tf.stack([self.vf,
                                      ...
                                      [-1, self.height, self.width, self.condition_dim + self.output_c_dim])
    ...
    def display(self,epoch,test_data,train_data):
        train_save_path = './Real-Time Display/'+str(epoch)+'_train/'
        test_save_path = './Real-Time Display/' + str(epoch) + '_test/'
        ...
        for id_test in range(idx_num):
            ...
            fake_top_2d, real_top = self.sess.run(
                [self.fake_B_sample, self.real_B], feed_dict=feed_test)
            ...
            for num in np.arange(self.batch_size):
                ...
                visualization(img,'test',id_test, num, test_save_path,save=1)
        idx_num = int(np.ceil(np.shape(train_data)[0] // self.batch_size))
        for id_test in range(idx_num):
            starter = id_test * self.batch_size
            end = starter + self.batch_size
            feed_test = {self.real_data: train_data[starter:end]}
            fake_top_2d, real_top = self.sess.run(
                [self.fake_B_sample, self.real_B], feed_dict=feed_test)
            fake_top = np.reshape(fake_top_2d, [-1, self.resolution])
            for num in np.arange(self.batch_size):
                ...
                img2 = np.reshape(real_top[num], (self.height, self.width))
                img = np.concatenate((img1, img2), axis=0)
                visualization(img, 'train', id_test, num, train_save_path, save=1)
        print(str(epoch)+' Real-time display has been established')
    ...
    def discriminator(self, image, y=None, reuse=False):
        with tf.variable_scope("discriminator"+self.flag) as scope:
            if reuse:
    ...
    def generator(self, image, y=None, is_training=True, reuse=False):
        with tf.variable_scope("generator"+self.flag) as scope:
    ...
    def train(self, args,data_out='none'):
        """Train pix2pix"""
        ...
        if self.restore_model == 'none':
            ...
            start_epoch = 100
            checkpoint_dir = os.path.join(self.checkpoint_dir, self.restore_model)
            model_name_num = self.restore_model + '-' + str(
                start_epoch)  # self.epoch_restore is the epoch number to restore
        ...
        for epoch in xrange(start_epoch,args.epoch):
            ...
            training_loss_writer(self.sample_dir, epoch, (time.time() - start_time)/60, self.model_name, (errGAN_D_real)/batch_idxs, (errGAN_D_fake)/batch_idxs, (errGAN_D_fake+errGAN_D_real)/batch_idxs, errG_final/batch_idxs, errGAN_G_eval/batch_idxs, vfae_eval/batch_idxs, mse_eval/batch_idxs, mae_eval/batch_idxs)
            ...
            if np.mod(epoch, 1) == 0 and epoch <= args.epoch:
                self.sample_model_validation(self.sample_dir, epoch, np_data_validation)
                self.sample_model_test(self.sample_dir, epoch, np_data_test)

            if np.mod(epoch, args.save_epoch_freq) == 0 or (epoch >= args.epoch-self.save_latest_freq):
                self.save(self.checkpoint_dir, epoch)

            if epoch<20 :
                self.display(epoch,np_data_test,np_data_train)





    def test_after_training(self, args ,data = 'none'):
        ...
        for id_test in range(idx_num):
            ...
            mse, mae, fake_top_2d, real_top = self.sess.run(
                [self.mse_sample, self.mae_sample, self.fake_B_sample, self.real_B], feed_dict=feed_test)

            fake_top = np.reshape(fake_top_2d, [-1, self.resolution])

            fake_total.extend(fake_top)
            real_total.extend(all_data[starter:end])
            for num in np.arange(self.batch_size):
                img1 = np.reshape(fake_top[num], (self.height, self.width))
                img2 = np.reshape(real_top[num], (self.height, self.width))
                img = np.concatenate((img1, img2), axis=0)
                visualization(img, self.flag, 'test', num + starter, image_save_path, save=1)
                print(self.flag, '_picture_', num + starter, ' : finish !')

        print('All projects completed !')

    def train_results_after_training(self, args ,is_test = False):
        ## path of the folder to save visualization result (note:need '/' at the end )
        ...
        test_sample_size = max(2000,2*self.batch_size) # Here 640 is the minimum test sample size which should be larger than the batch_size
        if is_test == True:
            all_data = self.get_sample_data(self.dataset_test, test_sample_size)
        else:
        ...
        for id_test in range(idx_num):
            ...
            for num in np.arange(self.batch_size):
                ...
                img2 = np.reshape(real_top[num], (self.height, self.width))
                img = np.concatenate((img1, img2), axis=0)
                visualization(img, self.flag , is_test, num+starter, image_save_path, save=1)
                print(self.flag,'_picture_',num+starter,' : finish !')
